...
def CCGBlockText(V, A, global_state, subgraph_size, text_features=None,
                 no_filters=64, act_func=tf.nn.relu, mask=None,
                 num_updates=None, is_training=1, name=None):
    with tf.compat.v1.variable_scope(name, default_name='CCGBlockText') as scope:
        dim_global_state = global_state.get_shape()[-1].value
        global_state = tf.reshape(global_state, [-1, int(dim_global_state / 2)])
        global_state_transfer = make_fc_layer(global_state, no_filters,
                                              act_func=act_func, with_bn=False,
                                              is_training=is_training)
        global_state_transfer = tf.reshape(global_state_transfer, [-1, 2, no_filters])

        if text_features is not None:
            text_dim = text_features.get_shape()[-1].value
            text_proj = make_fc_layer(text_features, no_filters,
                                      act_func=act_func, with_bn=False,
                                      is_training=is_training)
            text_proj = tf.reshape(text_proj, [-1, 2, no_filters])
        else:
            text_proj = tf.zeros_like(global_state_transfer)
        ...
        btext = broadcast_global_state(subgraph_size, text_proj, V_shape)

        V = tf.concat([V, bgs, btext], axis=-1)

        no_features_for_conv = no_filters * 2 + V_shape[-1]
        ...
        combined_global = tf.concat([global_state_transfer, text_proj], axis=-1)
        combined_global = tf.reshape(combined_global, [-1, no_filters * 2])
        combined_global = make_bn(combined_global, is_training, num_updates=num_updates)
        combined_global = act_func(combined_global)

        return V, combined_global

def EarlyFusionLayer(graph_features, text_features, hidden_dim=256, name=None):
    with tf.compat.v1.variable_scope(name, default_name='EarlyFusionLayer') as scope:
        graph_proj = make_fc_layer(graph_features, hidden_dim,
                                   act_func=tf.nn.relu, with_bn=True)
        text_proj = make_fc_layer(text_features, hidden_dim,
                                  act_func=tf.nn.relu, with_bn=True)

        fused = tf.concat([graph_proj, text_proj], axis=-1)

        return fused


def LateFusionLayer(graph_features, text_features, hidden_dim=256, name=None):
    with tf.compat.v1.variable_scope(name, default_name='LateFusionLayer') as scope:
        graph_proj = make_fc_layer(graph_features, hidden_dim,
                                   act_func=tf.nn.relu, with_bn=True)
        text_proj = make_fc_layer(text_features, hidden_dim,
                                  act_func=tf.nn.relu, with_bn=True)

        combined = tf.concat([graph_proj, text_proj], axis=-1)
        attention = make_fc_layer(combined, 2,
                                  act_func=tf.nn.softmax, with_bn=False)

        attention_expanded = tf.expand_dims(attention, -1)
        features_stack = tf.stack([graph_proj, text_proj], axis=1)
        fused = tf.reduce_sum(attention_expanded * features_stack, axis=1)
