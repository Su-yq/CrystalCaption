...
def select_vars(remove_var=['loss/max_acc_test:0', 'loss/max_acc_train:0'], remove_keywords=['FC', 'final'],
...
class Finetuning(Model):
    ...
    def make_train_step(self, optimizer='adam', starter_learning_rate=0.1, learning_rate_step=1000,
                        learning_rate_exp=0.1):
        if self.reports == None:
            self.reports = {}
        print('Preparing training')
        assert hasattr(self, 'loss'), "self.loss未在父类Model中定义！"
        assert isinstance(self.loss, tf.Tensor), f"self.loss应为TF Tensor，实际类型：{type(self.loss)}"
        assert self.loss.shape == (), f"self.loss应为标量（shape=()），实际shape：{self.loss.shape}"
        ...
        with tf.control_dependencies(update_ops):
            if optimizer == 'adam':
                ...
                learning_rate = tf.compat.v1.train.exponential_decay(starter_learning_rate,
                                                                     ...
                                                                     learning_rate_exp,
                                                                     staircase=True)
                self.train_step = tf.compat.v1.train.MomentumOptimizer(learning_rate, 0.9).minimize(self.loss,
                                                                                                    global_step=self.global_step,
                                                                                                    var_list=self.trainable_vars,
                                                                                                    name='train_step')
